...
{ 
    ...
    $paging = new PHPPaging;
	$sql = "select * from medium where lab='$consul2'";
}
           ...
           if($consul1=="Supermanager")                        
{
    ...
    ?>
        <tr id="tr_<?=$rs_per['Medium_ID']?>">
         
            <td><?=$rs_per['Name']?></td>
            <td><?=$rs_per['Alias']?>
            <td><?=$rs_per['Type']?></td>
             <td><?=$rs_per['Components']?></td>
            <td><?=$rs_per['Sterilization']?>
            <td><?=$rs_per['PH']?></td>
            <?php
           if($consul1=="Supermanager")                        
{
	 ...
	 <td><?php echo $consullab ?></td>
        ...
        <?php
            ...
            <td><a href="javascript: fn_mostrar_frm_modificar(<?=$rs_per['Medium_ID']?>);"><img src="../extras/ico/page_edit.png" /></a></td>
            <td><a href="javascript: fn_eliminar(<?=$rs_per['Medium_ID']?>);"><img src="../extras/ico/delete.png" /></a></td>
        </tr>
    <?php } ?>
    </tbody>
    <tfoot>
