<?php
 ...
 require'../extras/class/sessions.php';
	...
	$sql = sprintf("select * from extraction_method where Extraction_Method_ID=%d",
		(int)$_POST['ide_per']
	);
	$per = mysql_query($sql);
...
?>
<h3>Update Extraction Method</h3>

<form action="javascript: fn_modificar();" method="post" id="frm_per">
	<input type="hidden" id="ide_per" name="ide_per" value="<?=$rs_per['Extraction_Method_ID']?>" />
...
<form name="SubirProtocol" method="post" action="../subida_y_precarga_ajax-master/index.php"  >
       <table>
           <input type="hidden" id="id_assay" name="id_assay" value="<?=$rs_per['Extraction_Method_ID']?>" />
           <input type="hidden" id="type" name="type" value="ExtraMe" />
