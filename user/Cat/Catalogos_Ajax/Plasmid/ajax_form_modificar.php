<?php
	...
	$sql = sprintf("select * from plasmid where Plasmid_ID=%d",
		(int)$_POST['ide_per']
	);
	$per = mysql_query($sql);
...
<form action="javascript: fn_modificar();" method="post" id="frm_per" name="frm_per">
	<input type="hidden" id="ide_per" name="ide_per" value="<?=$rs_per['Plasmid_ID']?>" />
    <table class="formulario">
        <tbody>
             ...
             <tr>
              ...
              <tr>
                <td>Vector</td>
                <td><input name="S_type" type="text" id="S_type" size="25" class="cajas" value="<?=$rs_per['Vector']?>"  /></td>
            </tr>
            <tr>
                <td>Extraction Date</td>
                <td><input name="R_fecha" type="text" id="R_fecha"  value="<?=$rs_per['Fecha_Extraccion']?>"   /></td>
            </tr>
             <tr>
             <tr>
                <td>Storage</td>
