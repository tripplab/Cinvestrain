...
{ 
    ...
    $paging = new PHPPaging;
	$sql = "select * from role where lab='$consul2'";
    
}	if (isset($_GET['criterio_usu_per']))
		$sql .= " where Type like '%".fn_filtro(substr($_GET['criterio_usu_per'], 0, 16))."%'";
           ...
           if($consul1=="Supermanager")                        
{
	 ...
	 <td><?php echo $consullab ?></td>
        ...
        <?php
            ...
            <td><a href="javascript: fn_mostrar_frm_modificar(<?=$rs_per['Role_ID']?>);"><img src="../extras/ico/page_edit.png" /></a></td>
            <td><a href="javascript: fn_eliminar(<?=$rs_per['Role_ID']?>);"><img src="../extras/ico/delete.png" /></a></td>
        </tr>
    <?php } ?>
    </tbody>
    <tfoot>
        <tr>
            <td colspan="6">
				<?
					...
					Show: <?=$paging->numRegistrosMostrados()?> Records,  <?=$paging->numPrimerRegistro()?> to <?=$paging->numUltimoRegistro()?><br />
					of : <?=$paging->numTotalRegistros()?><br />
                ?>
