<?php
	...
	$sql = sprintf("select * from role where Role_ID=%d",
		(int)$_POST['ide_per']
	);
	$per = mysql_query($sql);
...
<form action="javascript: fn_modificar();" method="post" id="frm_per">
	<input type="hidden" id="ide_per" name="ide_per" value="<?=$rs_per['Role_ID']?>" />
    <table class="formulario">
        <tbody>
            ...
            <tr>
                <td>*Type</td>
                <td><input name="usu_per" type="text" id="usu_per" size="40" class="cajas" value="<?=$rs_per['Type']?>" required/></td>
            </tr>
