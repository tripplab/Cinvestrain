...
{ 
	...
	$sql = "select * from preservation_method";
}
else{
    $paging = new PHPPaging;
	$sql = "select * from preservation_method where lab='$consul2'";
}
	...
	if (isset($_GET['criterio_ordenar_por']))
		...
		$sql .= " order by Preservation_Method_ID desc";
           ...
           if($consul1=="Supermanager")                        
{
    ...
    ?>
        <tr id="tr_<?=$rs_per['Preservation_Method_ID']?>">
          
            <td><?=$rs_per['Name']?></td>
            <td><?=$rs_per['Alias']?>
            <td><?=$rs_per['Description']?></td>
            <?php
           if($consul1=="Supermanager")                        
{
                $labo=$rs_per['lab'];
	 ...
	 <td><?php echo $consullab ?></td>
        ...
        <?php
            ...
            <td><a href="javascript: fn_mostrar_frm_modificar(<?=$rs_per['Preservation_Method_ID']?>);"><img src="../extras/ico/page_edit.png" /></a></td>
            <td><a href="javascript: fn_eliminar(<?=$rs_per['Preservation_Method_ID']?>);"><img src="../extras/ico/delete.png" /></a></td>
        </tr>
    <?php } ?>
    </tbody>
    <tfoot>
