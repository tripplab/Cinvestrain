<?php
	...
	$sql = sprintf("select * from position where id_position=%d",
		(int)$_POST['ide_per']
	);
	$per = mysql_query($sql);
...
<form action="javascript: fn_modificar();" method="post" id="frm_per">
	<input type="hidden" id="ide_per" name="ide_per" value="<?=$rs_per['id_position']?>" />
    <table class="formulario">
        <tbody>
            ...
            <tr>
                <td>*Position</td>
                <td><input name="usu_per" type="text" id="usu_per" size="40" class="cajas" value="<?=$rs_per['position']?>" required/></td>
            </tr>
