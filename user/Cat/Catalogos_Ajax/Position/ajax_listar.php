...
if($consul1=="Supermanager")                        
{
    ...
    $paging = new PHPPaging;
	$sql = "select * from position where lab='$consul2'";
}
	if (isset($_GET['criterio_usu_per']))
		$sql .= " where position like '%".fn_filtro(substr($_GET['criterio_usu_per'], 0, 16))."%'";
	if (isset($_GET['criterio_ordenar_por']))
		$sql .= sprintf(" order by %s %s", fn_filtro($_GET['criterio_ordenar_por']), fn_filtro($_GET['criterio_orden']));
	else
		$sql .= " order by id_position desc";
           ...
           if($consul1=="Supermanager")                        
{
    ...
    ?>
        <tr id="tr_<?=$rs_per['id_position']?>">
            
            <td><?=$rs_per['position']?></td>
           ...
           if($consul1=="Supermanager")                        
{
	 ...
	 <td><?php echo $consullab ?></td>
        ...
        <?php
            ...
            <td><a href="javascript: fn_mostrar_frm_modificar(<?=$rs_per['id_position']?>);"><img src="../extras/ico/page_edit.png" /></a></td>
            <td><a href="javascript: fn_eliminar(<?=$rs_per['id_position']?>);"><img src="../extras/ico/delete.png" /></a></td>
        </tr>
    <?php } ?>
    </tbody>
    <tfoot>
