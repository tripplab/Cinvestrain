...
{ 
    ...
    $paging = new PHPPaging;
	$sql = "select * from host_info where lab='$consul2'";
           ...
           if($consul1=="Supermanager")                        
{
    ...
    ?>
        <tr id="tr_<?=$rs_per['ID_Host']?>">
            <td><a href="busMap_Act.php?id=<?=$rs_per['ID_Host']?>"><img src="../extras/ico/page_edit.png" /></a></td>
            <td><a href="javascript: fn_eliminar(<?=$rs_per['ID_Host']?>);"><img src="../extras/ico/delete.png" /></a></td>
            <td><?=$rs_per['Name']?></td>
		...
		while($resullab=mysql_fetch_array($cslab)){
			echo $consulStorage=$resullab[0];
             ...
             <td><?=$rs_per['Tissues_collected']?></td>
            <td><?=$rs_per['Physiological_state']?>
