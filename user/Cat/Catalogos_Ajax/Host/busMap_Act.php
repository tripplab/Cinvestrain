...
$u="ho";
		...
		while($resul=mysql_fetch_array($cs1)){
			$consul2=$resul[0];
                         ...
                         $sql = sprintf("select * from host_info where ID_Host=%d",
		(int)$_GET['id']
	);
	$per = mysql_query($sql);
	...
	$rs_per = mysql_fetch_assoc($per);
                         ...
                         $varFree=$rs_per['ID_Host'];
              $sqlCordenadas="SELECT Latitude,Longitude FROM host_info WHERE  ID_Host='$varFree'";		
...
{    
   ...
   var  numeroplan=document.getElementById("plan").value;
      var  numeroorg=document.getElementById("org").value;
       ...
       var c=document.getElementById("colle").value;
         var si=document.getElementById("sia").value;
          ...
          var pre=document.getElementById("pro").value;
             
          $.ajax({
         type:"POST",   
          url: "BD_A_Host.php",  
      data:{plan:numeroplan,org:numeroorg,com:co,Site_A:SiteA,Storage_id:Storage,sci:sc,gen:ge,spe:sp,date:da,col:c,sia:si,sid:s,sub:subir, labor:labo,pro:pre}
        });
             ...
             <br>
               <br>
                 <br>
                   <br>
                 <br>
       <center>   <h1>Host Information</h1></center>
...
<form  id="form_mapa" name="form_mapa" method="POST" action="" enctype="multipart/form-data">
    ...
    <input type="hidden" name="plan" id="plan"   value="<?=$rs_per['ID_Host']?>" >
    <br>
    <div> 
...
echo '</select>';
    ...
    <div>
     ...
     <div>
        ...
        <input class="cajas" type="text" name="date" id="date" value="<?=$rs_per['Altitude']?>" onKeyPress="return validar_monto(event)">MALS
    </div>
      <br>
    <div>
        <label>  Tissues collected </label>
         <?php 
        
                   echo"<select class='cajas'name=colle id=colle  onChange='mostrar(this.value);'>"; 
echo "<option value=".$rs_per['Tissues_collected'].">".$rs_per['Tissues_collected']."</option>\n"; 
                           $sql="SELECT type FROM type_host"; 
                           $result=mysql_query($sql); 
                           $i=0; 
      ...
      <div id="desdeotro" style="display:none;">
       <input class="cajas" name="pro" id="pro" type="text"   />
      ...
      <br>
        ...
        <?php 
                   ...
                   echo"<select class='cajas'name=sia id=sia>"; 
  echo "<option value=".$rs_per['Physiological_state'].">".$rs_per['Physiological_state']."</option>\n"; 
                           $sql="SELECT Status FROM physiological_state where lab='$consul2'"; 
                           $result=mysql_query($sql); 
                           $i=0; 
                 while ($row=mysql_fetch_row($result)) 
                      { 
