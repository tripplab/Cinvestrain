           ...
           if($consul1=="Supermanager")                        
{
	  ...
	  <th>Lab</th>
        ...
        <?php
            ...
            <th></th>
            <th width="16px"><a href="../vista/index.php"><img src="../../extras/ico/add.png"></a></th>
        </tr>
    </thead>
    <tbody>
    <?php
    ...
    ?>
        <tr id="tr_<?=$rs_per['User_ID']?>">
            <td><?=$rs_per['Nick']?></td>
            <td><?=$rs_per['Name']?></td>
            <td><?=$rs_per['Last_Name']?>
            <td><?=$rs_per['Telephone']?></td>
             <td><?=$rs_per['email']?></td>
            <td><?=$rs_per['Role']?>
            <td><?=$rs_per['position']?>
            <td><?=$rs_per['Status']?></td>
           ...
           if($consul1=="Supermanager")                        
{
	 ...
	 <td><?php echo $consullab ?></td>
        ...
        <?php
            ...
            <td><a class="fancybox fancybox.iframe" href="../vista/fancyBoxModificarUsuario.php?nombres=<?=$rs_per['Name']?>&apellidos=<?=$rs_per['Last_Name']?>&tel=<?=$rs_per['Telephone']?>&email=<?=$rs_per['email']?>&role=<?=$rs_per['Role']?>&position=<?=$rs_per['position']?>&status=<?=$rs_per['Status']?>&imagen=<?=$rs_per['photo']?>&pk=<?=$rs_per['User_ID']?>&Nick=<?=$rs_per['Nick']?>"><img src="../../extras/ico/page_edit.png" /></a></td>
            <td><a href="../control/controlador.php?eliminar_usuario=si&codigo=<?=$rs_per['User_ID']?>"><img src="../../extras/ico/delete.png" /></a></td>
        </tr>
    <?php } ?>
    </tbody> 
    <tfoot>
