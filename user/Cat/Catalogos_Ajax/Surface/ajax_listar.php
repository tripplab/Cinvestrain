...
{ 
	...
	$sql = "select * from surface_morphology ";
}
else{
    
    $paging = new PHPPaging;
	$sql = "select * from surface_morphology where lab='$consul2'";
}
	if (isset($_GET['criterio_usu_per']))
		$sql .= " where Form like '%".fn_filtro(substr($_GET['criterio_usu_per'], 0, 16))."%'";
	if (isset($_GET['criterio_ordenar_por']))
		$sql .= sprintf(" order by %s %s", fn_filtro($_GET['criterio_ordenar_por']), fn_filtro($_GET['criterio_orden']));
	else
		$sql .= " order by Surface_Morphology_ID desc";
           ...
           if($consul1=="Supermanager")                        
{
    ...
    ?>
        <tr id="tr_<?=$rs_per['Surface_Morphology_ID']?>">
             <td><?=$rs_per['Name']?></td>
            <td><?=$rs_per['Form']?></td>
            <td><?=$rs_per['Elevation']?>
            <td><?=$rs_per['Margin']?></td>
             <td><?=$rs_per['Appereance']?></td>
            <td><?=$rs_per['Level']?>
            <td><?=$rs_per['Pigmentation']?></td>
              <td><?=$rs_per['Color']?></td>
                         <?php
           if($consul1=="Supermanager")                        
{
	 ...
	 <td><?php echo $consullab ?></td>
        ...
        <?php
            ...
            <td><a href="javascript: fn_mostrar_frm_modificar(<?=$rs_per['Surface_Morphology_ID']?>);"><img src="../extras/ico/page_edit.png" /></a></td>
            <td><a href="javascript: fn_eliminar(<?=$rs_per['Surface_Morphology_ID']?>);"><img src="../extras/ico/delete.png" /></a></td>
        </tr>
    <?php } ?>
    </tbody> 
    <tfoot>
